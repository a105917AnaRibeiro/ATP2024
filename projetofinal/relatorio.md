...
1.2 ESTRUTURA DO PROGRAMA
    A aplicação foi organizada com uma interface gráfica que contém menus e botões para aceder às funcionalidades principais, tornando o uso mais intuitivo e eficiente. As opções disponíveis incluem:~

    -Criação de Publicações: Permite adicionar novas publicações científicas à base de dados. O utilizador pode inserir informações detalhadas, como título, autores, palavras-chave e DOI.
    -Atualização de Publicações: Editar as informações de uma publicação já existente, selecionando-a.
    -Visualização de Publicações: Inclui filtros e ordenação de publicações por critérios como título, autor, palavras-chave ou data de publicação. As publicações filtradas podem ser exibidas numa lista interativa.
    ...
    -Importação de Publicações: permite importar novos registos de outro ficheiro que tenha a mesma estrutura do ficheiro suporte (ata_medica_papers.JSON)
...
2.2.5 Importação de Publicações
    ...
    Lê o arquivo JSON selecionado pelo utilizador e carrega os dados nele contidos.
